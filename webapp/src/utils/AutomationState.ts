...
export const makeAumationStateEditor = (
  ...
  onUpdate,
  triggers: {
    update: (i: number, t: AutomationTrigger) =>
      onUpdate({
        ...automation,
        trigger: automation.trigger
          .slice(0, i)
          .concat([t])
          .concat(automation.trigger.slice(i + 1)),
      }),
    delete: (i: number) =>
      onUpdate({
        ...automation,
    ...
    add: (t: AutomationTrigger) =>
      onUpdate({
        ...automation,
        trigger: automation.trigger.concat([t]),
      }),
  },
  updateSequence: (s: AutomationSequenceNode[]) =>
    onUpdate({
      ...automation,
      action: s,
    }),
