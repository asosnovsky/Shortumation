...
export const getDescriptionFromAutomationNode = <N extends AutomationNodeTypes>(
    ...
    if ('platform' in node) {
        ...
        if (node.platform === 'device') {
            return node.device_id ? namer.getDeviceName(node.device_id) : node.entity_id ? namer.getEntityName(node.entity_id) : node.platform
        }
    ...
    if ('condition' in node) {
        if (
            ...
            switch (node.condition) {
                case "numeric_state":
                    if (node.entity_id) {
                        const center = (Array.isArray(node.entity_id) && node.entity_id.length > 1) ?
                            '...' :
    ...
    if ('device_id' in node) {
        return `${node.type ?? ""} on ${node.device_id ? namer.getDeviceName(node.device_id) : ''}`
    }
    if ('choose' in node) {
        return "Choose"
    }
