...
import { ParallelAction } from "./actions";
...
test("AutomationTimeString is valid", () => {
  for (const c of ["10:23", "15:02", "13:00:1", "23:00:23"]) {
...
test("AutomationTimeString is invalid", () => {
  for (const c of ["xx:23", "1x:02", "13:0a:1c", "25:00:23", "11"]) {
    const failures = getFailures(
      {
        time: c,
      },
...
test("Node Validation for parallel", () => {
  expect(
    getFailures(
      {
        parallel: { bad: "me" },
